   ...
   if( $("#upload_services_json").exists() ){
	 ...
	 /*SEARCH CODE*/
   	...
   	$( document ).on( "change", "#actions", function() {	
		$actions = $("#actions").val();   		
		customBannerSwitch($actions);
	});	
	
   	if ((typeof  current_page !== "undefined") && ( current_page !== null)) {   		
   		if(current_page=="home_banner_new" || current_page=="home_category_new"){
   			$actions = $("#actions").val();   		
   			customBannerSwitch($actions);
   		}
   	}
...
customBannerSwitch = function($actions){
	$(".input_tags").hide();
	$(".input_custom_page").hide();
	$(".input_custom_link").hide();
	
	switch($actions){
		case "custom_page":
		  $(".input_custom_page").show();
		break;
		
		case "custom_link":
		  $(".input_custom_link").show();
		break;
