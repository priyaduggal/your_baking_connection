...
class LocationWrapper
{
	...
	public static $active_merchant_id=0;
	...
	public static function GetStateList($page=0, $page_limit=10, $search_string='')
	{
		...
		if(!empty($search_string)){
   	    ...
   	    if(self::$active_merchant_id>0){
   	    	$and.=" AND a.state_id IN (
   	    	   select state_id from {{location_rate}}
   	    	   where merchant_id=".q(self::$active_merchant_id)."
   	    	)
   	    	";
   	    }
	...
	public static function GetLocationCity($page=0, $page_limit=10, $search_string='', $state_id=0)
	{
		...
		$and="";
   	    ...
   	    if(self::$active_merchant_id>0){
   	    	$and.=" AND a.city_id IN (
   	    	   select city_id from {{location_rate}}
   	    	   where merchant_id=".q(self::$active_merchant_id)."
   	    	)
   	    	";
   	    }
	...
	public static function GetAreaList($city_id='',$page=0, $page_limit=10, $search_string='')
	{
		...
		if(!empty($search_string)){
   	    ...
   	    if(self::$active_merchant_id>0){
   	    	$and.=" AND a.area_id IN (
   	    	   select area_id from {{location_rate}}
   	    	   where merchant_id=".q(self::$active_merchant_id)."
   	    	)
   	    	";
   	    }
	...
	public static function getDeliveryFee($merchant_id='',$fee=0,$state_id='', $city_id='',$area_id='',$all=false)
	{
		if($merchant_id<=0){
			return false;
		}
		...
		$stmt="
		SELECT a.fee,a.minimum_order,a.free_above_subtotal
