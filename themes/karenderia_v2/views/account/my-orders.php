...
<div class="card p-0 p-lg-3 mb-3 border d-none" >
 ...
 <div  :class="{ 'resultfound': hasResults, 'notfound': !hasResults }"  >Records not found</div>
<!--KMRS ROW-->
<template v-for="datas in data" >
       ...
       <table class="table tbbox">
               <thead>
                              <tr>
                                 ...
                                 <th scope="col" style="width: 190px;">Connect with Baker</th>
                                 <th scope="col">Action</th>
                              </tr>
                           </thead>
        <tbody>
        
        <tr v-for="order in datas" >
            
            <td><div class="tbl_cart_product_thumb"><img class="rounded-pill" v-if="merchants[order.merchant_id]" :src="merchants[order.merchant_id].url_logo"/></div></td>
            <td>{{order.order_id}} </td>
             <td>{{order.date_created}}</td>
                <td>{{ order.total }}</td>
                  <td><span v-if="status[order.status]" 
            class="">
   ...
   <span v-else class="">
     ...
     <td class="text-center">
         <a  href="javascript:void(0)"  data-email="merchants[order.merchant_id].email" target="_blank" data-toggle="tooltip" title="Connect with Baker" 
     class="btn btn-success addbtn ccustomer mt-2"><i class="fa fa-envelope"></i></a></td>
      <td>
                                    <div class="loginbox">
                                      <a class="btn btn-primary btn-lg btn-theme" :href="order.view">
	   <?php echo t("View")?>
	    </a> </div>
                                 </td>
        </tr>
...
<div class="kmrs-row row m-0 rounded p-2 mb-2 d-none" v-for="order in datas" >
 <div class="col p-0">
     ...
     <div class="flex-fill">
        <div class="d-flex align-items-center">
         <div class="align-self-center mr-2">
            <h6 class="m-0" v-if="merchants[order.merchant_id]" >{{ merchants[order.merchant_id].email }} {{ merchants[order.merchant_id].restaurant_name }}</h6>
         </div>
 ...
 <div class="col">
  <div class="d-flex">
    <div>
      ...
      <p class="text-grey m-0"><?php echo t("Place on")?> 
    ...
    <div class="dropdown dropleft">	      
	   ...
	   <div class="dropdown-menu dropdown-actions" aria-labelledby="dropdownMenuLink">	    
	    ...
	    <!--</a>			    -->
	     ...
	     <a class="dropdown-item ssm-toggle-nav" :href="order.view"  target="_blank">
	    <i class="zmdi zmdi-eye mr-2"></i> <?php echo t("View")?>
	    </a>
...
<template v-if="!loading">
	...
	<div class="d-flex justify-content-center mt-2 mb-2" v-if="show_next_page" >
...
:label="{
  ...
  what_did_you_like: '<?php echo CJavaScript::quote(t("What did you like? asdsd"))?>',  
  what_did_you_not_like: '<?php echo CJavaScript::quote(t("What did you not like? sdas"))?>',
